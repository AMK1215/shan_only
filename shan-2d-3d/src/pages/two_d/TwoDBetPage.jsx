...
const TwoDBetPage = () => {
    ...
    const colorOptions = [
        { id: 1, color: "#3B82F6", text: "ထိုးငွေပမာဏ" },
    ];
    ...
    return (
        <div className="min-h-screen animated-bg particle-bg pb-28 twoD-page">
            <ToastContainer />
            {/* Top Action Buttons */}
            <div className="max-w-md mx-auto flex justify-between items-center pt-6 pb-2 px-6 gap-4">
                <div className="flex flex-col items-center">
                    <button 
                        ...
                        className="w-14 h-14 flex items-center justify-center rounded-full bg-gray-800/80 backdrop-blur-md shadow-lg border border-gold/30 hover:scale-105 active:scale-95 transition-all"
                    >
            ...
            <div className="w-full max-w-sm mx-auto px-2">
                <div className="glass-effect rounded-3xl shadow-2xl border border-gold/30 p-6 mb-10 flex flex-col gap-3">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm font-bold text-white modern-subtitle">ပိတ်ရက်ချိန်</span>
                    <select className="modern-input px-3 py-2 rounded-xl border border-gold/30 bg-gray-800/80 text-sm font-bold text-white focus:outline-none focus:ring-2 focus:ring-gold shadow-sm">
                      <option>12:00 PM</option>
                      <option>04:30 PM</option>
                    </select>
                  </div>
                  <div className="text-sm text-gray-200 mb-2 modern-subtitle">လောင်းကြေးထည့်ပါ။</div>
                  <input
                    type="number"
                    value={amount}
                    onChange={e => setAmount(Number(e.target.value))}
                    className="modern-input w-full px-6 py-4 rounded-2xl border border-gold/30 bg-gray-800/80 text-xl text-center font-bold text-white focus:outline-none focus:ring-2 focus:ring-gold shadow-lg"
                    placeholder="100"
                    min={1}
                  />
                </div>
                ...
                {/* Number Grid */}
                <div className="glass-effect rounded-3xl shadow-2xl border border-gold/30 mb-8 p-6 relative hover-lift">
                    {/* Info icon for color codes */}
                    <button onClick={handleShow} className="absolute top-3 right-3 w-7 h-7 flex items-center justify-center rounded-full bg-gradient-to-br from-gold/80 to-dark-gold/80 shadow border border-gold/50 hover:scale-110 transition-all">
                    ...
                    <div className="mb-4 text-center">
                        <h3 className="text-lg font-bold text-white mb-1">ဂဏန်းရွေးချယ်ပါ (00-99)</h3>
                        <p className="text-sm text-gray-300">Scroll down to see all numbers</p>
                    </div>
                    
                    {/* Scrollable Number Grid */}
                    <div className="number-grid-container rounded-3xl border border-gold/30 bg-gray-800/70 p-4 shadow-inner glass-effect">
                            ...
                            {numbers_static.two_digits.map((item, idx) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`twoD-number-card flex flex-col items-center justify-center rounded-xl px-0 py-2 border-2 focus:outline-none select-none transition-all duration-300
                                            ${isSelected ? 'active' : ''}
                                        `}
                                    >
                                        <span className="font-black text-lg text-white drop-shadow-sm tracking-wide modern-title">{item.two_digit}</span>
                                        <div className="w-8 h-1.5 rounded-full mt-1.5 mb-1 overflow-hidden bg-gray-600 modern-progress">
                                            <div className="h-full rounded-full transition-all duration-500 modern-progress-bar" style={{ width: `${Math.min(percent, 100)}%` }}></div>
                                        </div>
                                    </button>
                                );
                            })}
                        ...
                        {/* Scroll indicator */}
                        <div className="text-center mt-4 pt-2 border-t border-gold/20">
                            <p className="text-xs text-gray-400">Showing all 100 numbers (00-99)</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed bottom-20 left-1/2 -translate-x-1/2 w-full max-w-sm px-4 z-40">
                    <button
                        className="w-full py-4 rounded-3xl twoD-fab text-gray-900 font-black text-xl shadow-2xl hover:scale-105 active:scale-95 transition-all tracking-wide drop-shadow-lg"
                        onClick={() => {
            ...
            {/* Modern Glassy Modal */}
            <Modal show={show} onHide={handleClose} centered contentClassName="!rounded-3xl !bg-gray-800/90 !backdrop-blur-xl !shadow-2xl !border !border-gold/30">
                <Modal.Header closeButton className="!border-b-0 !rounded-t-3xl !bg-gray-800/80">
                    <Modal.Title>
                        <span className="font-bold text-xl text-white modern-title">အရောင်ရှင်းလင်းချက်</span>
                    </Modal.Title>
                </Modal.Header>
                <Modal.Body className="flex flex-col gap-4 p-6">
                    <span className="text-center font-bold text-sm text-white modern-subtitle">ထီထိုးငွေ ၁၀၀% ပြည့်ပါက ဂဏန်းပိတ်ပါမည်။</span>
                    <div className="flex flex-col gap-3 mt-2">
                        {colorOptions.map((item) => (
                            <div key={item.id} className="flex items-center gap-3">
                                ...
                                ></div>
                                <span className="text-sm text-white font-semibold modern-subtitle">{item.text}</span>
                            </div>
                        ))}
                    </div>
                </Modal.Body>
                <Modal.Footer className="flex justify-end gap-3 p-4 !rounded-b-3xl !bg-gray-800/80">
                    <button onClick={handleClose} className="px-5 py-2 rounded-xl bg-gold/80 text-gray-900 font-semibold shadow hover:bg-gold transition-all">Close</button>
                </Modal.Footer>
            </Modal>
        </div>
    );
