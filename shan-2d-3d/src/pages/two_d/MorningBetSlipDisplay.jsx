...
import MorninBetSlipDetailComponent from './MorninBetSlipDetailComponent.jsx';

// BetSlipDisplay Component: Displays the details of a two-digit bet slip.
// It receives the bet slip data as a prop.
const MorningBetSlipDisplay = () => {
  const { data, loading, error } = useFetch(`${CONFIG.API_BASE_URL}${CONFIG.ENDPOINTS.TWO_D_BET_SLIPS}`);
  const [selectedSlipId, setSelectedSlipId] = useState(null);

  const slips = useMemo(() => Array.isArray(data) ? data : [], [data]);
  ...
  if (selectedSlip) {
    return (
      <MorninBetSlipDetailComponent slip={selectedSlip} onBack={() => setSelectedSlipId(null)} />
    );
  }
  ...
  return (
    <div className="p-6 bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl mx-auto text-white font-inter border border-gray-700">
      <h2 className="text-2xl font-bold text-teal-400 mb-4">မနက်ပိုင်း စလစ်စာရင်း</h2>
        ...
        {slips.map((slip) => (
          ...
          >
            <div>
              ...
              <p className="text-gray-300 text-sm">ကစားသမား: {slip.player_name} • ငွေပမာဏ: ${parseFloat(slip.total_bet_amount).toFixed(2)}</p>
            </div>
            <div className="text-right">
              <p className={`text-sm font-semibold capitalize ${slip.status === 'pending' ? 'text-yellow-400' : 'text-green-400'}`}>{slip.status}</p>
              <p className="text-gray-400 text-xs">{slip.game_date} {slip.game_time}</p>
            </div>
          </button>
        ))}
      </div>
...
export default MorningBetSlipDisplay;
